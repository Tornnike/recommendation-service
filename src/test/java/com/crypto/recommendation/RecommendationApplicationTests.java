...
class RecommendationApplicationTests {
	...
	void getNormalRange() throws Exception {
		...
		mockMvc.perform(get("/recommendation-service/normal-range/"))
	...
	void getMonthly() throws Exception {
		...
		mockMvc.perform(get("/recommendation-service/monthly/1"))
				.andExpect(status().isOk())
	...
	void getByCrypto() throws Exception {
		...
		mockMvc.perform(get("/recommendation-service/crypto/BTC"))
				.andExpect(status().isOk())
